 ...
 * under the License.
 */
package ai.houyi.dorado.netty.ext;
...
import ai.houyi.dorado.netty.ext.HttpPostBodyUtil.SeekAheadOptimize;
import ai.houyi.dorado.netty.ext.HttpPostBodyUtil.TransferEncodingMechanism;
import ai.houyi.dorado.netty.ext.HttpPostRequestDecoder.EndOfDataDecoderException;
import ai.houyi.dorado.netty.ext.HttpPostRequestDecoder.ErrorDataDecoderException;
import ai.houyi.dorado.netty.ext.HttpPostRequestDecoder.MultiPartStatus;
import ai.houyi.dorado.netty.ext.HttpPostRequestDecoder.NotEnoughDataDecoderException;
...
public class HttpPostMultipartRequestDecoder implements InterfaceHttpPostRequestDecoder {
	...
	/**
	 * Set from the request ContentType the multipartDataBoundary and the possible
	 * charset.
	 */
	...
	private void checkDestroyed() {
		if (destroyed) {
			throw new IllegalStateException(
					HttpPostMultipartRequestDecoder.class.getSimpleName() + " was destroyed already");
		}
	}
	...
	/**
	 * Set the amount of bytes after which read bytes in the buffer should be
	 * discarded. Setting this lower gives lower memory usage but with the overhead
	 * of more memory copies. Use {@code 0} to disable it.
	 */
	@Override
	...
	/**
	 * Return the threshold in bytes after which read data in the buffer should be
	 * discarded.
	 */
	@Override
	...
	/**
	 * True if at current getStatus, there is an available decoded InterfaceHttpData
	 * from the Body.
	 *
	...
	/**
	 * Returns the next available InterfaceHttpData or null if, at the time it is
	 * called, there is no more available InterfaceHttpData. A subsequent call to
	 * offer(httpChunk) could enable more data.
	 *
	 * Be sure to call {@link InterfaceHttpData#release()} after you are done with
	 * processing to make sure to not leak any resources
	 *
	 ...
	 * @param closeDelimiterStatus the next getStatus if the delimiter is a close
	 *                             delimiter
	...
	private Attribute getContentDispositionAttribute(String... values) {
		...
		// Filename can be token, quoted or encoded. See
	...
	protected InterfaceHttpData getFileUpload(String delimiter) {
		...
		if (currentFileUpload == null) {
			...
			try {
				...
				currentFileUpload = factory.createFileUpload(request, cleanString(nameAttribute.getValue()),
	...
	/**
	 * Remove all Attributes that should be cleaned between two FileUpload in Mixed
	 * mode
	 */
	...
	/**
	 * Read one line up to --delimiter or --delimiter-- and if existing the CRLF or
	 * LF Read one line up to --delimiter or --delimiter-- and if existing the CRLF
	 * or LF. Note that CRLF or LF are mandatory for opening delimiter (--delimiter)
	 * but not for closing delimiter (--delimiter--) since some clients does not
	 * include CRLF in this case.
	 *
	...
	/**
	 * Read one line up to --delimiter or --delimiter-- and if existing the CRLF or
	 * LF. Note that CRLF or LF are mandatory for opening delimiter (--delimiter)
	 * but not for closing delimiter (--delimiter--) since some clients does not
	 * include CRLF in this case.
	 *
	 * @param delimiter of the form --string, such that '--' is already included
	 ...
	 *         closing)
	 * @throws NotEnoughDataDecoderException Need more chunks and reset the
	 *                                       readerInder to the previous value
	 */
